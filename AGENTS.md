...
- When the user asks to `部署预览` or deploy a preview for this project, run the local build first when practical, then deploy to Vercel production with `vercel deploy --prod --yes` so the stable project URL `https://moocky-frontend-engineering-shell.vercel.app/` updates as well.
- Treat one-off preview deployments without the stable alias as an explicit exception. Only use `vercel deploy --yes` without `--prod` when the user specifically asks for a temporary preview URL.
- After deployment, verify the stable URL returns `200 OK` before reporting completion.
- After creating or modifying a prototype and successfully deploying it, update the `Latest Prototype Preview` entry below with the stable URL, deployment URL, date, and a short note about what changed.

### Latest Prototype Preview

- 2026-04-28: Stable `https://moocky-ai.vercel.app/`; deployment `https://moocky-frontend-engineering-shell-n4y23dx5g.vercel.app/`; `/ai` is live with visible answer deltas, application-composed Personalized Suggestion cards, early `cards_ready`, and stable stream-to-final reconciliation without answer-block flashing.
